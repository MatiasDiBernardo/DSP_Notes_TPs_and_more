...
def estimateInharmonicity(inputFile = '../../sounds/piano.wav', t1=0.1, t2=0.5, window='hamming', 
    ...
    fs2 = xHfreq.shape[0] * fs/len(x)  #Get samplre rate of the frecuency bins
    t1Index = int(fs2 * t1)
    t2Index = int(fs2 * t2)

    # 3. Compute the mean inharmonicity of the segment
    inharmonicity = np.zeros(xHfreq.shape[0])
    for l in range(xHfreq.shape[0]):
        R = xHfreq.shape[1]  #Num of armonics expected
        f0 = xHfreq[l, 0]
        ...
        for r in range(1,R):
            if xHfreq[l, r] != 0:
                inharmValue += abs(xHfreq[l, r] - (r + 1) * f0)/float(r + 1)
            else:
                zeroValInharmonic += 1

        inharmonicity[l] = inharmValue/(R - zeroValInharmonic)

    inharmMean = 1/(t2Index - t1Index + 1) * np.sum(inharmonicity[t1Index:t2Index+1])
