...
def optimalZeropad(x, fs, f):
    ...
    """
    periodSamples = int(1/f * fs)

    rest = len(x)/periodSamples
    rest = Decimal(str(rest)) % 1  #Get only decimal part
    zeroPadAdd = periodSamples * (1 - float(rest))

    """
    Better way to handel this
    res = fs / f
    ...
    xPadded = np.concatenate([x, np.zeros(int(zeroPadAdd))])
    
    calculatDFT = fft(xPadded)

    return 20* np.log10(abs(calculatDFT[:len(xPadded)//2 + 1]))
