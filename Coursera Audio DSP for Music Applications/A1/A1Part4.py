...
def downsampleAudio(inputFile, M):
    ...
    newSampleRate = samplerate // M  #Procentaje of the new samplerate
    windowAverage = int(samplerate/newSampleRate)  #Proportion between sr
    numIterations = len(data)//windowAverage  #Num iterarions to take average

    for i in range(int(numIterations)):
        newValue = np.mean(data[i * windowAverage : i * windowAverage + windowAverage])

        downSampleSignal.append(newValue)

    return np.array(downSampleSignal)
