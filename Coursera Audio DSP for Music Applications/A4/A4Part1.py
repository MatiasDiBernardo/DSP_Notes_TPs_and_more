...
def extractMainLobe(window, M):
    ...
    fftWin = fft(w, N)
    fftWin = fftshift(fftWin)
    fftWin = abs(fftWin)

    idxMax = np.argmax(fftWin)

    prevIdx = fftWin[idxMax]
    count = 1
    nextIdx = fftWin[idxMax + count]
    ...
    upperIndex = idxMax + count
    lowerIndex = idxMax - count + 1  #Explois symmetry

    winLobe = fftWin[lowerIndex:upperIndex]
    winLobe += eps

    return 20 * np.log10(winLobe)
